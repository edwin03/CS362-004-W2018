...
void testSmithy() {
  ...
  for (i = 0; i < NUM_TESTS; i++) {
    ...
    for (j = 0; j < game.numPlayers; j++) {
      ...
      game.handCount[j] = j == player ? randInt(0, MAX_HAND-4): randInt(0, MAX_HAND);
    ...
    if (asserttrue(cardsDrawn == 3, ASSERT_EXIT) == 0) {
      testsFailed++;
      printf(": player %d - 3 cards drawn (and 1 card discarded)\n", player);
    }
    else {
      printf(": player %d - 3 cards drawn (and 1 card discarded)\n", player);
